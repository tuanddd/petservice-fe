...
export default props => {
  ...
  const remove = id => {
    let name = prompt("Type the virus name to confirm", "");
    let confirmed = name === state.name;
    if (confirmed) {
      api.delete(`${API.VIRUSES}/${id}`).then(res => {
        history.push("/viruses");
      });
    } else {
  ...
  useEffect(() => {
    if (id) {
      api.get(`${API.VIRUSES}/${id}`).then(async res => {
        try {
  ...
  return (
    <div className="flex flex-col mx-6 my-4 flex-1 h-mc">
      ...
      >
        ...
        <span className="ml-1 text-lg">Back to virus list</span>
      </Link>
      <h1 className="text-2xl text-gray-700">
        {id ? `Virus - ${state.name}` : "New virus"}
      </h1>
      <div className="bg-gray-400 mb-8 mt-2 h-px"></div>
      <Form
        onSubmit={() => {
          if (id) {
            api.put(`${API.VIRUSES}/${id}`, state).then(res => {
              history.push("/viruses");
            });
          } else {
            api.post(API.VIRUSES, state).then(res => {
              history.push("/viruses");
            });
          }
        }}
            ...
            <div className="flex">
              <div className="w-1/3 pr-2">
                ...
                <p className="text-sm text-gray-600 mt-3">
                  Remove this virus from the system. This action cannnot be
                  undo.
                </p>
              </div>
