...
export default props => {
  ...
  const remove = id => {
    let name = prompt("Type the breed name to confirm", "");
    let confirmed = name === state.name;
    if (confirmed) {
      api.delete(`${API.BREEDS}/${id}`).then(res => {
        history.push("/breeds");
      });
    } else {
  ...
  useEffect(() => {
    if (id) {
      api.get(`${API.BREEDS}/${id}`).then(async res => {
        try {
  ...
  return (
    <div className="flex flex-col mx-6 my-4 flex-1 h-mc">
      ...
      >
        ...
        <span className="ml-1 text-lg">Back to breed list</span>
      </Link>
      <h1 className="text-2xl text-gray-700">
        {id ? `Breed - ${state.name}` : "New breed"}
      </h1>
      <div className="bg-gray-400 mb-8 mt-2 h-px"></div>
      <Form
        onSubmit={() => {
          if (id) {
            api.put(`${API.BREEDS}/${id}`, state).then(res => {
              history.push("/breeds");
            });
          } else {
            api.post(API.BREEDS, state).then(res => {
              history.push("/breeds");
            });
          }
        }}
